...
import imagesUrls from '../meetups/queries.json';

const postsDirectory = path.join(process.cwd(), 'posts');
const queriesPath = path.join(process.cwd(), 'meetups/queries.json');

function writeData(data) {
	fs.writeFileSync(queriesPath, JSON.stringify(data));
	console.log(imagesUrls);
}

function getData(id) {
	console.log(
		imagesUrls.find((iu) => iu._id === id),
		'getData'
	);
	return imagesUrls.find((iu) => iu._id === id);
}
